 ...
 */
router.post('/media', protect, adminOnly, mediaUpload.single('file'), async (req, res) => {
  if (!req.file) {
 ...
 */
router.post('/avatar', protect, avatarUpload.single('file'), async (req, res) => {
  if (!req.file) {
    return res.status(400).json({ message: 'No file provided' });
  }
  try {
